...
uniform sampler2D[MAX_PROBE_COUNT]          radianceProbeGrid;
uniform sampler2D[MAX_PROBE_COUNT]          normalProbeGrid;
uniform sampler2D[MAX_PROBE_COUNT]          distanceProbeGrid;
uniform sampler2D[MAX_PROBE_COUNT]          lowResolutionDistanceProbeGrid;
uniform ivec3                               probeCounts;
uniform vec3                                probeStartPosition;
uniform vec3                                probeStep;
uniform int                                 lowResolutionDownsampleFactor;
uniform sampler2D[MAX_PROBE_COUNT]          irradianceProbeGrid;
uniform sampler2D[MAX_PROBE_COUNT]          meanDistProbeGrid;
...
vec3 gridCoordToPosition(in LightFieldSurface L, GridCoord c) {
    return L.probeStep * vec3(c) + L.probeStartPosition;
}


vec3 probeLocation(in LightFieldSurface L, ProbeIndex index) {
